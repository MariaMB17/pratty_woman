...
class Estudiantes extends CI_Controller{
  ...
  public function guardarStudent (){ 
    ...
    /*$curso = substr($this->input->post("cursos"),0,5);
   
    $fecha = substr($this->input->post("fecha"),0,1);*/
    
    echo $cedula.'-'.$apellidos.'-'.$nombres.'-'.$telefono.'-'.$direccion.'-'.$correo.'-'.$curso_servicio.'-'.$costo.'-'.
        $fecha_citado.'-'.$disponibilidad.'-'.$condicion_pago.'-'.$bancos.'-'.$transaccion.'-'.$numero.'-'.$pago_monto.'-'.$fecha_pago;
    if($cedula !=null && 
       $apellidos !=null && 
       $nombres !=null && 
       $telefono !=null && 
  ...
  public function modificar_cursos_estudiante () {
    ...
    $costo = $this->input->post("costo");    
    $curso_1 = substr($this->input->post("curso_1"),0,1);    
    ...
    $id_cronograma = $fecha;
     ...
     echo $cedula.'---'.$id_cabecera.'--'.$curso.'--'. $costo.'--'. $curso_1.'--';
     echo $costo_1.'--'. $fecha.'--'. $disponibilidad.'--'. $condicion_pago.'--'.$bancos.'--'.$transaccion.'--';      
     echo $numero.'--'. $pago_monto.'--'. $fecha_pago.'--'.$id_curso.'--'. $abono .'--'. $id_detalles.'--';  
         
     if($cedula != null && $id_cabecera !=null &&
       $curso != null && $costo !=null && $curso_1 =="u" &&
       $costo_1 =="undefined" && $fecha !=null && $disponibilidad =="undefined" &&
       ...
       $this->form_validation->set_rules('costo','costo','trim|required|decimal|xss_clean');
       $this->form_validation->set_message('required', 'Debe completar este campo');
       $this->form_validation->set_rules('curso_1','cursos','trim|required|xss_clean');
       $this->form_validation->set_message('required', 'Debe completar este campo');
       $this->form_validation->set_rules('costo_1','costo','trim|required|decimal|xss_clean');
       $this->form_validation->set_message('required', 'Debe completar este campo'); 
       $this->form_validation->set_rules('fecha','fecha','trim|required|integer|max_length[3]|xss_clean');
       $this->form_validation->set_message('required', 'Debe completar este campo');
       $this->form_validation->set_rules('disponible','disponible','trim|required|integer|max_length[2]|xss_clean');
